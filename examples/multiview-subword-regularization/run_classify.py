...
def train(args, train_dataset, model, tokenizer, lang2id=None, dropped_train_dataset=None):
  """Train the model."""
  ...
  if dropped_train_dataset is not None:
...
def main():
  ...
  parser.add_argument(
    ...
    type=str,
    required=True,
    help="Path to pre-trained model or shortcut name selected in the list: " + ", ".join(['bert-base-multilingual-cased']),
  )
  parser.add_argument(
  ...
  parser.add_argument("--mvr",
    action="store_true",
    help="Whether to enable training with Multiview Subword Regularization.",
  )
  parser.add_argument("--bpe_dropout", 
          type=float,
          default=0, 
          help="A float number between 0 and 1. Set subword regularization strength for standard training."
          "A value of 0 indicates no subword regularization."
  )
  parser.add_argument("--mvr_bpe_dropout", 
          type=float,
          default=0, 
          help="A float number between 0 and 1. Set subword regularization strength for Multiview Subword Regularization."
          "A value of 0 indicates no subword regularization."
  )
  parser.add_argument("--kl_weight", 
          type=float,
          default=0, 
          help="A float number indicating the weight of consistency loss for Multiview Subword Regularization."
  )
  parser.add_argument("--kl_t", 
          type=float,
          default=1, 
          help="The temperature term for scaling the softmax distribution for Multiview Subword Regularization."
  )
  ...
  if args.do_train:
    ...
    train_dataset = load_and_cache_examples(args, args.task_name, tokenizer, split=args.train_split, language=args.train_language, lang2id=lang2id, bpe_drop=args.bpe_dropout, evaluate=False)
    if args.mvr:
      dropped_train_dataset = load_and_cache_examples(args, args.task_name, tokenizer, split=args.train_split, language=args.train_language, lang2id=lang2id, bpe_drop=args.mvr_bpe_dropout, evaluate=False)
    else:
      dropped_train_dataset = None
    global_step, tr_loss, best_score, best_checkpoint = train(args, train_dataset, model, tokenizer, lang2id, dropped_train_dataset)
